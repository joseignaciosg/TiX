  ...
  namespace :python do
    ...
    after :deploy, :copy_files do
      on roles(:app) do
        ...
        execute "cp -rv #{fetch(:deploy_to)}/current/TiX/PythonApp/ServerApp/* #{fetch(:install_to)} || echo"
        execute "cd #{fetch(:install_to)} && git checkout ./data_processing/ || echo"
      end
    end
    ...
    after :copy_files, :restart_service do
      on roles(:app) do
        ...
        execute "sudo /etc/init.d/serverTiX-#{env} stop"
        execute "sudo /etc/init.d/serverTiX-#{env} start"
      end
    end
  end
